...
public class GeneralEnemy : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        max_next = rootpos[(int)startpos].transform.childCount;//子供の数取得
    }
    void Generation(int num)
    {
        //パーティクル生成
        ParticleSystem new_particle = Instantiate(enemy_particle[num], rootpos[(int)startpos].transform);
        new_particle.Play();

        Vector3 offset = new Vector3(0, prefab[num].transform.localScale.y ,0);//キャラの高さ分
        GameObject enemy = Instantiate(prefab[num], rootpos[(int)startpos].transform.position + offset, new Quaternion(0, 180.0f, 0, 1));//生成
        time = 0;//時間リセット
        enemy_count++;//敵カウント
    }
