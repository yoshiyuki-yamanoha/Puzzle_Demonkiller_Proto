            ...
            //if (Vector3.Distance(Vector3.zero, tf.position) > 0.3f)
            {
                ...
                foreach (GameObject o in circles)
                {
                    ...
                    while (per < 1.0f)
                    {
                        ...
                        if (Vector3.Distance(currentPerPos, o.transform.position) < dist && oldOverlapObject != o && o.gameObject.activeInHierarchy)
                        {
                            //最近選択していたオブジェクト
                            oldOverlapObject = o;
                    ...
                    if (Vector3.Distance(tf.position, o.transform.position) < dist && o.gameObject.activeInHierarchy)
                    {
