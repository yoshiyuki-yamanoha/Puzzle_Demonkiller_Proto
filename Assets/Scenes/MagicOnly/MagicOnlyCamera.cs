...
public class MagicOnlyCamera : TrunManager
{
    //public
    ...
    {
        if (magicOnly || /*(Input.GetButtonDown("Fire3") && MSCameraflag == false)||*/ trunMgr.GetTrunPhase() == TrunPhase.MagicAttack)
        {
        ...
        else if (magicOnly || /*(Input.GetButtonDown("Fire3") && MSCameraflag == true)||*/ trunMgr.GetTrunPhase() == TrunPhase.Enemy)
        {
            if (MSCameraflag == true)
            {
                MSCameraflag = false;
    ...
    public float CalcMoveRatio()
    {
        ...
        {
            startTime = Time.time;
            if (magicOnly || trunMgr.GetTrunPhase() == TrunPhase.Enemy)
            {
                speed = maAtakSelectSpeed + 1;
            }
            else if (magicOnly || trunMgr.GetTrunPhase() == TrunPhase.MagicAttack)
            {
