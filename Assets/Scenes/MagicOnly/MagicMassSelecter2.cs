...
public class MagicMassSelecter2 : MonoBehaviour
{
    ...
    [SerializeField] MagicRangeDetector2 s_MagicRangeDetector;
    ...
    //選ぶ方法 (0:マス  1:敵)
    int selectType = 0;
    KariEnemyStatus2 currentSelectEnemy = null;
    ...
    {
        if (magicOnlyMode || s_TrunManager.trunphase == TrunManager.TrunPhase.MagicAttack && s_OrbGage.OrbCheckExsistens())
        {
    ...
    public void SwitchSelectType(int num = 99)
    {
        ...
        if (selectType == 1)
        {
            ...
            {
                int xBuf = e.GetComponent<KariEnemyStatus2>().x;
                int yBuf = e.GetComponent<KariEnemyStatus2>().y;
                ...
                if (difTortal <= oldDifTotal)
                {
                    ...
                    nearX = xBuf;
                    nearY = yBuf;
                    currentSelectEnemy = e.GetComponent<KariEnemyStatus2>();
                }
            }
    ...
    void MoveSelecterEnemy()
    {
        ...
        if (oldX != x || oldY != y)
        {
            ...
            //
            if(!magicOnlyMode) sePlay.Play("MagicCursorSelect");
    ...
    (int, int) GetNearEnemyX(int vecNum)
    {
        ...
        while (currentSelectEnemy == null)
        {
            ...
            foreach (var e in enemies)
            {
                ...
                KariEnemyStatus2 s_Enemy = e.GetComponent<KariEnemyStatus2>();
    ...
    GameObject GetEnemyObjectOnCurrentSelectMass()
    {
        ...
        foreach (var e in enemies)
            if (e.GetComponent<KariEnemyStatus2>().x == nowSelX && e.GetComponent<KariEnemyStatus2>().y == nowSelY) return e;
    ...
    {
        if (!magicOnlyMode) sePlay.Play("MagicCursorSelect");//フィールドのマスを動かす時に音を鳴らす
        s_MapMass.SetMagicMassSelector(nowSelX, nowSelY);
    }
            ...
            if (typ != 2)
            {
                if (Input.GetButtonDown("Fire1"))
                {
                    ...
                    if (typ == 0 || typ == 1 || typ == 4 || typ == 3)
                    {
                        ...
                        if (!magicOnlyMode) sePlay.Play("MagicShot");
                    }
    ...
    void CalcBottomCost()
    {
        ...
        if (selectTargets[0].GetComponent<KariEnemyStatus2>() == null) return;
    ...
    (int, int) GetOldSelectedEnemyPos()
    {
        ...
        //直近選択した敵の座標
        KariEnemyStatus2 s_EnemyOld = selectTargets[selectsNum - 1].GetComponent<KariEnemyStatus2>();
        int x = s_EnemyOld.x;
        int y = s_EnemyOld.y;
